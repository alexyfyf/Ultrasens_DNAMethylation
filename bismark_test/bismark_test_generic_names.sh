...
log() {
  printf '[%s] [bismark_test_generic_names] %s\n' "$(date '+%Y-%m-%d %H:%M:%S')" "$*" >&2
}
...
SAMPLE_DIR="$DATA_DIR/${SAMPLE}_generic_names"
...
"$PYTHON_BIN" "$ROOT_DIR/scripts/plot_cgi_methylation_bivariate.py" \
...
"$PYTHON_BIN" "$ROOT_DIR/scripts/analyze_cpg_density_switch.py" \
  ...
  --matlab-style-plot "$FIGURES_DIR/individual_cpg_methylation_heatmap_and_classes_by_density.png" \
  --label "$SAMPLE ${ANALYSIS_SUFFIX}"
...
finish_step "$FIGURES_DIR/individual_cpg_methylation_heatmap_and_classes_by_density.png"

log "generating mean/median/Hill CpG-density plot"
"$PYTHON_BIN" "$ROOT_DIR/scripts/plot_cpg_density_methylation_summary.py" \
...
"$PYTHON_BIN" "$ROOT_DIR/scripts/plot_cpg_switch_parameters.py" \
  --inputs "$RESULTS_DIR/individual_cpg_methylation_density_${ANALYSIS_SUFFIX}.bed" \
  --labels "$SAMPLE" \
  --cohorts "$SAMPLE" \
  --ko-types None \
